...
"meta": {
  ...
  "desktopWidth": 600,
  "mobileWidth": 360,
  "verbatimSections": ["section-7-legal", "section-9-disclaimer"],
  "openQuestions": [
    {"id": "cta-url", "question": "Confirm production URL for primary CTA", "blocking": true}
  ]
}
```
...
- `verbatimSections` — section IDs whose text content must never be paraphrased. Use for any regulated, legal, or otherwise word-locked copy (terms of service, ISI in pharma, legal disclaimers, financial disclosures, etc.). The extraction skill flags these; the coding skill renders them verbatim.
- `openQuestions` — items the extraction skill couldn't resolve confidently. A TODO list for human review.
...
**Verbatim rule:** for any section listed in `meta.verbatimSections`, the concatenated `text` values across the section's textBlocks MUST match Figma exactly. Word-for-word, including curly apostrophes, special punctuation, and line breaks. Never summarize or paraphrase verbatim content.
...
- `href` — real URL or `"#"` placeholder. Production launch needs real URLs.
- `alias` — tracking name (ESP-specific; e.g., SFMC uses this for click reporting). Must be unique across the email.
...
**Verbatim content**
- For any section in `meta.verbatimSections`, every textBlock and list item in that section's nodes must have non-empty text runs. The skill cannot produce verbatim sections with placeholder or empty content.
