...
3. Coherence          — multi-column overflow, gap references,
                        alias uniqueness, verbatim content presence
"""
import json
import re
import sys
...
def validate(spec: dict) -> List[dict]:
    ...
    verbatim_sections = set(meta.get("verbatimSections", []))
    ...
    for i, section in enumerate(sections):
        ...
        # Verbatim content check: every textBlock and list in a verbatim section
        # must have non-empty text in every run.
        if section.get("id") in verbatim_sections:
            text_runs_in_section = list(walk_text_runs(section["nodes"]))
            if not text_runs_in_section:
                warn(
                    f"{spath}.nodes",
                    f"section '{section['id']}' is in verbatimSections but contains no text runs"
                )
            for run in text_runs_in_section:
                if not run.get("text"):
                    err(
                        f"{spath}.nodes(textRun)",
                        f"section '{section['id']}' is in verbatimSections but has an empty text run"
                    )
